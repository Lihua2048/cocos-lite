...
import { SDFTextRenderer } from "./sdf-font/sdf-text-renderer";
...
export class WebGLRenderer {
    ...
    async initialize(canvas) {
        ...
        if (this.shaderProgram) {
            this.uProjectionMatrixLocation = this.gl.getUniformLocation(this.shaderProgram, "u_projectionMatrix");
            this.uColorLocation = this.gl.getUniformLocation(this.shaderProgram, "u_color");
            this.uTextureLocation = this.gl.getUniformLocation(this.shaderProgram, "u_texture");
            this.uUseTextureLocation = this.gl.getUniformLocation(this.shaderProgram, "u_useTexture");
            this.aPositionLocation = this.gl.getAttribLocation(this.shaderProgram, "a_position");
            // 创建实体顶点缓冲区
  ...
  `;
    ...
    render(entities, animations, entityAnimationState) {
        ...
        if (currentWidth !== this.lastCanvasSize.width ||
            currentHeight !== this.lastCanvasSize.height) {
            // 更新投影矩阵
            mat4.ortho(this.projectionMatrix, 0, currentWidth, currentHeight, 0, -1, 1);
            this.lastCanvasSize = { width: currentWidth, height: currentHeight };
        }
        // 激活着色器程序，将投影矩阵传递给gpu
        gl.useProgram(shaderProgram);
        if (this.uProjectionMatrixLocation) {
            gl.uniformMatrix4fv(this.uProjectionMatrixLocation, false, this.projectionMatrix //相当于transform
            );
        }
        // 提前设置顶点属性指针
        ...
        entities.forEach((entity) => {
            ...
            if ((entity.type === 'ui-button' || entity.type === 'ui-input' || entity.type === 'ui-text') && this.sdfTextRenderer && this.sdfFontLoaded && this.sdfShaderProgram) {
                ...
                if (text) {
                    ...
                    // 绘制文字
                    this.sdfTextRenderer.drawText(text, x, textY, {
                        fontSize,
                        color: textColor,
                        textAlign,
                        maxWidth: width
        ...
        // 关键帧插值函数
        function interpolateKeyframes(keyframes, t, key) {
