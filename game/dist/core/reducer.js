    ...
    switch (action.type) {
        ...
        case "ADD_ENTITY": {
            ...
            if (entity.type === 'ui-button' || entity.type === 'ui-input' || entity.type === 'ui-text') {
                ...
                // color 兜底
                let safeColor = [0.9, 0.9, 0.9, 1];
                if (Array.isArray(props.color) && props.color.length === 4 && props.color.every((v) => typeof v === 'number' && !isNaN(v))) {
                    safeColor = [props.color[0], props.color[1], props.color[2], props.color[3]];
                }
                // backgroundType 只允许 'color' | 'image'
                let safeBackgroundType = 'color';
                if (props.backgroundType === 'image')
                    safeBackgroundType = 'image';
                // textColor 兜底
                let safeTextColor = [0, 0, 0, 1];
                if (Array.isArray(props.textColor) && props.textColor.length === 4 && props.textColor.every((v) => typeof v === 'number' && !isNaN(v))) {
                ...
                if (typeof props.fontSize === 'number' && !isNaN(props.fontSize))
                    safeFontSize = props.fontSize;
                // textAlign 只允许 'left' | 'center' | 'right'
                let safeTextAlign = 'left';
                if (props.textAlign === 'center' || props.textAlign === 'right')
                    safeTextAlign = props.textAlign;
                entity.properties = {
                    width: props.width,
                    height: props.height,
                    color: safeColor,
        ...
        case "UPDATE_ENTITY": {
            ...
            if (existingEntity.type === 'sprite') {
                ...
                const merged = {
                    ...
                    ...filterUndefined(updates.properties || {}),
                };
                let safeColor = [1, 1, 1, 1];
                if (Array.isArray(updates.properties?.color) && updates.properties.color.length === 4 && updates.properties.color.every((v) => typeof v === 'number')) {
                    safeColor = [
                        updates.properties.color[0],
                        updates.properties.color[1],
                        updates.properties.color[2],
                        updates.properties.color[3],
                    ];
                }
                else if (Array.isArray(merged.color) && merged.color.length === 4 && merged.color.every((v) => typeof v === 'number')) {
                ...
                // backgroundType 只允许 'color' | 'image'
                let safeBackgroundType = 'color';
                if (merged.backgroundType === 'image')
                    safeBackgroundType = 'image';
                // textColor 兜底，且防止 NaN
                let safeTextColor = [0, 0, 0, 1];
                if (Array.isArray(merged.textColor) &&
                    merged.textColor.length === 4 &&
                    merged.textColor.every((v) => typeof v === 'number' && !isNaN(v))) {
                ...
                if (typeof merged.fontSize === 'number' && !isNaN(merged.fontSize))
                    safeFontSize = merged.fontSize;
                // textAlign 只允许 'left' | 'center' | 'right'
                let safeTextAlign = 'left';
                if (merged.textAlign === 'center' || merged.textAlign === 'right')
                    safeTextAlign = merged.textAlign;
                const uiProps = {
                    width: merged.width,
                    height: merged.height,
                    color: safeColorUI,
        ...
        case "UPDATE_PHYSICS_COMPONENT": {
            ...
            if (!entity)
                return state;
            const newComponents = entity.components.map(c => c.type === 'physics' ? { ...c, ...updates } : c);
            return {
                ...state,
                entities: {
                    ...state.entities,
